        ...
        <div class="    rounded-lg dark:border-gray-700">
            <main class="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
                ...
                <div v-if="stats" class="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>Immatriculations Special</CardTitle>
                        </CardHeader>
                        <CardContent>{{ stats['Immatriculation-Special'] ?? 0 }}</CardContent>
                    </Card>
...
async function fetchStats() {
    try {
        ...
        if (start && end) {
            ...
            response = await axios.get('/pdc/dashboard/stats/by/date', {
                params: {
                    start_date: startStr,
                    end_date: endStr,
                },
            ...
            response = await axios.get('/pdc/dashboard/stats')
        }
...
async function fetchGlobalStats() {
    try {
        const response = await axios.get("/pdc/dashboard/global/stats");
        ...
        // 🛠️ Fonction utilitaire pour transformer un objet brut en datasets
