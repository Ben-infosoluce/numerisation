...
async function handleModifier(dossier) {
    ...
    // Vérifie que le message est bien rempli
    if (!confirmMessage.value || confirmMessage.value.trim() === '') {
        toast.error("Veuillez saisir un message de confirmation.");
        return;
    }
    ...
    const payload = {
        ...
        message: confirmMessage.value.trim(), // 👈 ton message ici
    };

    console.log("Payload :", payload);

    try {
        const response = await axios.post("/correction/new", payload);

        console.log("Correction envoyée avec succès :", response.data);
        selectedFields.value = [];
        confirmMessage.value = ""; // Réinitialiser le champ message
        toast.success('Correction envoyée avec succès !');
        setTimeout(() => {
            window.location.href = `/numerisation/get/data/${props.dossier.num_chrono}`;
        }, 1500);
    } catch (error) {
