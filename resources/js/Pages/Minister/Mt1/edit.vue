        ...
        <div v-if="showModalEdit" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-3xl w-full max-h-[80vh] overflow-auto relative">
                ...
                <div class="w-full flex justify-center py-10">
                    <Input type="file" :accept="acceptedImageTypes" @input="handleFileUpload" />
                </div>
...
async function submitUpdate() {
    if (!fileToUpload.value || !currentDocumentId.value || !currentKey.value) {
        toast.error("Veillez renseignez le fichier")
        return
    }
    console.log(currentKey.value)


    let formatDataSingle = {
        document_id: currentDocumentId.value,
        [currentKey.value]: fileToUpload.value,
    ...
    try {
        ...
        setTimeout(() => {
            ...
            router.visit(`/numerisation.dossier.edit.documentslist/${props.dossier.id}`);
            //window.location.href = `/numerisation.dossier.edit.documentslist/${props.dossier.id}`;
        }, 1500);
        setTimeout(() => {
